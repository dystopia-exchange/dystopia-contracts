...
describe("bribe tests", function () {
  ...
  before(async function () {
    ...
    staking = await sr.deploy(mimUstPair.address, core.token.address) as StakingRewards;
  ...
  it("getPriorBalanceIndex for unknown token return 0", async function () {
    expect(await bribeMimDai.getPriorBalanceIndex(Misc.ZERO_ADDRESS, 0)).is.eq(0);
  });
